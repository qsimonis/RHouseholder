...
parameters{
    ...
    positive_ordered[Q] shrinkage_variance;
    vector[D] mu;
...
model{
    mu ~ normal(0, 10);
    view1_noise ~ exponential(10);
    view2_noise ~ exponential(10);
    for(i in 1:Q){
      shrinkage_variance[i] ~ cauchy(0,1);
      sigma_weight[i] ~ normal(0, shrinkage_variance[i]);
    }
    v ~ normal(0,1);
